...
FILTER_REASON_TITLES: dict[str, str] = {
    ...
    # Ниже — причины, которые до сих пор доходили до экрана кодом. Пользователь
    # видел `RAIL_SERVICE_CLASS_MISMATCH` и не мог понять, потерял он половину
    # выборки по делу или из-за дефекта.
    "COMPONENT_NOT_OBSERVED": "Компонента не наблюдается этим сценарием",
    "CABIN_CLASS_MISMATCH": "Не эконом-класс: сценарий считает массовый сегмент",
    "REFUNDABLE_FARE": "Возвратный тариф: методика считает невозвратные",
    "NOT_DIRECT": "Рейс с пересадкой: методика считает прямые",
    "RAIL_SERVICE_CLASS_UNKNOWN": "Купе, но класс обслуживания источник не сообщил",
    "RAIL_SERVICE_CLASS_MISMATCH": "Купе, но не того класса, что задан методикой",
    "ROOM_CATEGORY_EXCLUDED": "Апартаменты или хостел: методика считает гостиницы",
}

#: Что означает исход обращения к источнику для цифры на экране.
OUTCOME_TITLES: dict[str, str] = {
    ...
    "SCHEMA_ERROR": "Ответ источника разобрать не удалось — вероятно, изменился формат",
    "TRANSPORT_ERROR": "Обращение не состоялось: сеть или ошибка источника",
