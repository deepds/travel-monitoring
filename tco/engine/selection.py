...
def run_selection(
    ...
    Отбор начинается с чистого листа. Пометки предыдущего расчета снимаются:
    иначе повторный расчет не может вернуть в выборку то, что однажды из нее
    выбыло. Каждый шаг конвейера пишет причину исключения, но ни один не
    возвращает предложение обратно — это нормально внутри одного прохода и
    неверно между проходами.

    Без сброса пересчет исторического снимка новой методикой давал ровно тот же
    результат, что и старой: правила применялись, но их вердикт «предложение
    подходит» не мог перебить запись прошлого расчета. Замысел неизменяемого
    снимка, который можно пересчитать другими правилами, при этом не работал —
    и по цифрам это было не видно.
    """
    stats = SelectionStats(total=len(offers))
    for offer in offers:
        offer.exclusion_reason = ExclusionReason.NONE.value
        offer.exclusion_detail = None
