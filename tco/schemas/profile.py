...
class FilterRules(BaseModel):
    ...
    #: Допустимые классы обслуживания ЖД (``2К``, ``2Д`` и подобные) внутри уже
    #: отобранного типа вагона. Пустой список — без ограничения. Нужен там, где
    #: методика считает не «купе вообще», а конкретные категории: цены внутри
    #: купе различаются вдвое между двухместным и четырехместным.
    rail_service_classes: list[str] = Field(default_factory=list)
    #: Учитывать только невозвратные авиатарифы. Возвратные продаются с
    #: надбавкой, и в одной выборке с невозвратными завышают медиану.
    flights_non_refundable_only: bool = False
    #: Учитывать только прямые рейсы. Строже, чем ``max_stops=0``: последний
    #: задан на профиле как верхняя граница, а этот флаг выражает требование
    #: методики и читается в отчете как отдельное правило.
    flights_direct_only: bool = False
    #: Категории номеров, не относящиеся к массовому гостиничному сегменту.
    #:
    #: Апартаменты и хостелы исключены по прямому указанию: это другой продукт
    #: с другой ценой. Фильтр по типу объекта источник принимает, но сам тип в
    #: ответе не возвращает (поле пусто у всех 84 объектов казанской выдачи),
    #: поэтому проверить состав выборки можно только по названию категории
    #: номера. В казанской выборке апартаментов 158 из 1682 (9,4 %) против
    #: 3 из 2318 (0,1 %) в московской, по 12–14 тысяч рублей за ночь.
    #:
    #: Сравнение по вхождению подстроки без учета регистра. Пустой список —
    #: правило выключено.
    accommodation_excluded_room_keywords: list[str] = Field(default_factory=list)
