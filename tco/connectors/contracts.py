...
class TransportQuery:
    ...
    def is_one_way(self) -> bool:
        """Поездка в одну сторону: обратного плеча не запрашиваем.

        Выражается совпадением дат, а не отдельным полем. Отдельное поле
        пришлось бы включить в отпечаток сценария — иначе поездка «туда» и
        поездка «туда-обратно» на те же даты получили бы один отпечаток, — а
        совпадение дат уже в нем есть и означает ровно это: обратного плеча
        нет. Поездка «туда и обратно в тот же день» такой записью не
        выражается, и это осознанное ограничение: витрина ее не наблюдает.

        Одностороннее плечо нужно произвольным датам. Круговой тариф
        наблюдается только на каноническую длительность, поэтому на других
        датах возврата цифры нет вовсе; сумма двух плеч дает ее для любого
        интервала. Для ЖД это к тому же вдвое дешевле по обращениям: круговой
        сбор — это два поиска и до 16 запросов карты мест.
        """
