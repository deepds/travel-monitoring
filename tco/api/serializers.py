...
def run_brief(run: ScenarioRun) -> dict[str, Any]:
    return {
        ...
        "total_p25": _money(run.total_p25),
        "total_p75": _money(run.total_p75),
        "total_min": _money(run.total_min),
        "total_max": _money(run.total_max),
    ...
    return {
        **run_brief(run),
        ...
        "total": {
            ...
            "min": _money(run.total_min),
            "p25": _money(run.total_p25),
            "p75": _money(run.total_p75),
            "max": _money(run.total_max),
            "price_per_person": _money(run.price_per_person),
