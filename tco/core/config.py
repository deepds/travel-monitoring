...
class Settings(BaseSettings):
    ...
    #: До скольких страниц выдачи дочитывать там, где источник ее разбивает.
    #:
    #: Поиск отдает не больше тридцати объектов за раз и не сортирует их, а
    #: значит одна страница — это случайные тридцать. Замер 6 августа: 95 %
    #: запросов по отелям и 90 % по авиа упирались в потолок, по Казани полный
    #: обход менял медиану с 7 984 на 6 923 рубля. Десять — предел схемы
    #: инструмента; фактически обход прекращается раньше, на первой неполной
    #: странице (в среднем около пяти на город).
    collection_max_pages: int = 10

    #: Бюджет времени одного обращения к источникам внутри задачи сбора, секунды.
    #:
    #: Меньше предела времени самой задачи (300 с мягкий, 360 жесткий): предел
    #: снимает задачу целиком вместе с уже полученными предложениями, а бюджет
    #: позволяет прекратить перебор добровольно и сохранить собранное, пометив
    #: результат частичным.
    collection_time_budget_seconds: int = 240
